...
def save_param(CompiledDict, experiment, now):
    ...
    text_list.append('***Pre-Training a Material Reservoir with DE.***')
    text_list.append('Pre-Training using:')
    text_list.append('  > IntpScheme: %s' % (ParamDict['pt_IntpScheme']))
    text_list.append('  > FitScheme: %s' % (ParamDict['pt_FitScheme']))
    text_list.append('Final output layer trained using Ridged Regression')
    text_list.append('  > IntpScheme: %s' % (ParamDict['pt_final_IntpScheme']))
    text_list.append('  > FitScheme: %s\n' % (ParamDict['pt_final_FitScheme']))
    ...
    text_list.append('\nNumber of loops = %d' % (CompiledDict['num_systems']*CompiledDict['num_repetitions']))
    text_list.append('  > Number of generated circuits = %d' % (CompiledDict['num_systems']))
    text_list.append('  > Number of times the generated circuits where tested (num_repetitions) = %d' % (CompiledDict['num_repetitions']))

    text_list.append('Iteratiosn per loop = %d' % (ParamDict['epochs']))
    ...
    text_list.append('num input nodes = %d' % (NetworkDict['num_input']))
    text_list.append('num configuration nodes = %d' % (NetworkDict['num_config']))
    text_list.append('num output nodes = %d' % (NetworkDict['num_output']))
    ...
    if SpiceDict['model'] == 'NL_RN':
        ...
        text_list.append(' > Using distributions A~N(%f, %f^2) & B~N(%f, %f^2), two distributions A & B are generated using np.random.multivariate_normal with covariance factor = %f' % (SpiceDict['a_mean'],
                                                                                                                                                                                   SpiceDict['a_std'],
                                                                                                                                                                                   SpiceDict['b_mean'],
                                                                                                                                                                                   SpiceDict['b_std'],
                                                                                                                                                                                   SpiceDict['corr']))
