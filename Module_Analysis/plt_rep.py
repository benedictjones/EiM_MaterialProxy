...
def Plt__Av_FvIt(DirList, Save_NotShow=0, show=1, fill=0, ExpErrors=0,
    ...
    for curr_dir in range(len(new_DirList)):
        ...
        cir_MEAN_veri_fit = []
        cir_std_list = []
        for cir in range(MetaData['exp_num_circuits']):

            if MetaData['exp_num_repetitions'] > 1:
                rep_fit_list = []
                rep_pop_mean_fit_list = []
                rep_pop_std_fit_list = []
                rep_veri_fit_list = []
                for rep in range(MetaData['exp_num_repetitions']):
            ...
            if MetaData['exp_num_circuits'] > 1:
