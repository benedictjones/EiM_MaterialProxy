...
class Animation(object):
    ...
    def Plt__ani(self, Save_NotShow=0, PlotOnly='all', Specific_Cir_Rep='all', format='gif'):
        ...
        for curr_dir in new_DirList:
            ...
            cir_range = range(MetaData['exp_num_circuits'])
            rep_range = range(MetaData['exp_num_repetitions'])

            if MetaData['MG_animation'] == 0:
            ...
            if len(Specific_Cir_Rep) != 2:
                ...
                if Specific_Cir_Rep[0] >= MetaData['exp_num_circuits']:
                    print("Error: Invalid Circuit loop selected to plot")
                    return
                elif Specific_Cir_Rep[1] >= MetaData['exp_num_repetitions']:
            ...
            for cir in cir_range:
                ...
                for rep in rep_range:
                    ...
                    data_X, class_Y = Load_Data('train' , MetaData['num_input'],
                                                MetaData['num_output_readings'],
                                                MetaData['training_data'],
                                                MetaData['TestVerify'],
                                                MetaData['UseCustom_NewAttributeData'])
    ...
    def basic_ani(self, cir, rep, my_cmap, the_extent, data_X, class_Y, Save_NotShow=0):
        ...
        def animate(frame):
            ...
            ax.set_title("Animation (File %d, cir=%d, rep=%d) - Iteration %d, Best Fit = %.4f\nBest Genome: %s" % (self.dir_loop, cir, rep, frame, self.all_data__best_fit[frame], str(np.around(self.all_data__best_genome[frame],decimals=2))))
    ...
    def sub_ani(self, cir, rep, my_cmap, the_extent, data_X, class_Y, Save_NotShow=0):
        ...
        ax_mg.set_ylabel("$a_2$")
        ax_mg.set_xlabel("$a_1$")
