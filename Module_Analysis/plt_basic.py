...
class plotting(object):
    ...
    def Plt_basic(self, sel_dict='na', Save_NotShow=0, show=1, fill=0, ExpErrors=0,
        ...
        for self.curr_dir in self.new_DirList:
            ...
            exp_pop_size.append(self.MetaData['popsize'])
            ...
            for cir in cir_range:
                for rep in rep_range:
                    ...
                    with h5py.File(location, 'r') as hdf:
                        fit_list.append(np.array(hdf.get('/DE_data/fitness')))
                        pop_mean_fit_list.append(np.array(hdf.get('/DE_data/pop_mean_fit')))
                        self.pop_std_fit_list.append(np.array(hdf.get('/DE_data/pop_std_fit')))
                        if self.MetaData['TestVerify'] == 1:
                            self.veri_fit_list.append(np.array(hdf.get('/DE_data/veri_fit')))

            cont = self.MetaData['exp_num_circuits'] + self.MetaData['exp_num_repetitions']
    ...
    def plt_mean(self):
        if 'plt_mean' in self.sel_dict:
            if self.sel_dict['plt_mean'] == 1:
                ...
                fig = plt.figure(1)
                if self.title == 'on':
                    if self.PlotVeri == 1 and self.MetaData['TestVerify'] == 1:
