...
from mod_analysis.plt_basic import plotting
from mod_analysis.plt_ani import Animation
from mod_analysis.plt_mg import material_graphs
...
class analysis(Animation, plotting, material_graphs):
    ...
    def __init__(self, dir_list, dir_RefNameList=0, save_loc=0, format='png', dpi=400):
        ...
        if isinstance(dir_list, list) is True:
            self.MultiDirAly = 1
            if save_loc == 0:
                raise ValueError("Error (Analysis.py): Must use a custom save file for analysis on multiple files")
        else:
            self.MultiDirAly = 0
