...
class InterpretVoltages(object):
    ...
    def run(self, Vout, genome, data_Y, the_data, cir, rep):
        ...
        if str(data_Y) == 'na':
            ...
            if self.ptype == 'EiM':
                results = compute_EiM_ouputs(genome, Vout, data_Y, self.CompiledDict, the_data=the_data)
            else:
                raise ValueError("Currently only support EiM interp.")
    ...
    def _interpter_EiM_(self, Vout, genome, data_Y, the_data):
        ...
        class_out, responceY, Vout = compute_EiM_ouputs(genome, Vout, data_Y, self.CompiledDict, the_data=the_data)

        # get a fitness score
        scheme_fitness, err_fit, error = GetFitness(data_Y, class_out, responceY, Vout, self.CompiledDict)
